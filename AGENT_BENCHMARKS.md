...
repository, with and without Graphweft's MCP server — measuring the *marginal, end-to-end*
...
|---|---|---|
| A — baseline | No graphweft MCP server | — |
| B — graph-only | graphweft MCP, `GRAPHWEFT_CACHE_DIR` → empty dir, `GRAPHWEFT_EMBED_RUNTIME=off` | none |
| C — hybrid | graphweft MCP, default cache with prebuilt `jinaai/jina-embeddings-v2-base-code` index (695 chunks) | graph + embeddings |

Controls:

- Agents keep **all native tools** (grep, file read, shell) in every arm. This answers the
  honest question — "does adding graphweft help a real agent?" — rather than a rigged
...
- Claude uses `--strict-mcp-config` with a per-arm config file (arm A = empty server list),
  ...
  augments the user config, which contains no graphweft server (it does contain unrelated
...
- Both graphweft arms build the graph index at server spawn (in-memory); only the persistent
...
The organic matrix exposed an adoption gap: **Claude never called a graphweft tool
...
prompt ("Use the graphweft MCP tools … to locate the code before reading files"):
...
solved without graphweft** (0/3 organic Claude arms, 0/1 baseline Copilot) — was solved
...
**1. The hardest conceptual task is where graphweft pays.** `vector-prune` ("where are
...
71 s/$0.107 and 27 tool calls before giving a wrong answer. With graphweft MCP attached,
...
**3. Efficiency signal favors graphweft, with a caveat.** Copilot: graphweft arms cut wall
...
baseline ($0.028 vs $0.057/run) — but since it made 0 graphweft calls there, some of that
...
CLAUDE.md / copilot-instructions line like *"prefer graphweft_semantic_search for
...
trusting a wrong-but-plausible chunk. The `graphweft_semantic_search` output already
...
|---|---|---|
| Conceptual queries | hybrid hit@5 43% vs 14% graph-only | hardest conceptual task solved *only* with graphweft |
...
- Copilot exposes credits + token counts but not turn/tool-call detail; graphweft tool-call
