...
Tools: `graphweft_context` (ranked context for a task — hybrid with embeddings when an index
exists), `graphweft_semantic_search` (embedding-based conceptual search with line-level hits),
`graphweft_embed` (build/refresh the local embedding index), `graphweft_impact` (blast radius),
...
**Local semantic search (CLI + MCP):** `graphweft embed` builds an on-device embedding index —
AST-aware chunks (one per function/class) embedded with a bundled ONNX model
(`@huggingface/transformers`, downloaded once to `~/.graphweft/models`), persisted per repo.
`graphweft semantic "<query>"` returns the conceptually closest code with file:line ranges and
snippets, and `graphweft search` fuses semantic similarity into its ranking automatically. No
